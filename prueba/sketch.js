...
//salirse dle laberinto y sin tocar las zanahorias 
//hay que pasar los tres niveles 
// si uno pierde, le toca al otro personaje (jugador)
//el primero en pasar los tres niveles gana 
...
function draw() {
  ...
  if (estado == 1) {
    ...
    text("2. El oso empieza jugando. Si eres este, llega hasta la comida mas rica para pasar al siguiente nivel", 20 * width / 1000, 100 * height / 700);
    text("3. Si eres el zorro, llega hasta los vegetales para pasar al siguiente nivel", 20 * width / 1000, 140 * height / 700);
    text("4. Si pierdes, le toca jugar al otro jugador", 20 * width / 1000, 180 * height / 700);
    text("5. OJO! si tocas comida voladora, pierdes", 20 * width / 1000, 220 * height / 700);
    text("6. Si intentas hacer trampa y salirte del laberinto, tambien vas a perder", 20 * width / 1000, 260 * height / 700);
    text("8. El primero en terminar los tres niveles, es el ganador!", 20 * width / 1000, 300 * height / 700);
    text("9. Suerte! disfruta tu comida", 20 * width / 1000, 340 * height / 700);
    textSize(20);
  ...
  if (estado == 6) {
    ...
    cakee.dibujarse()
    cakee.moverse()
    if (dist(mouseX, mouseY, cakee.x, cakee.y) < 12) {
      estado = 12;
    }
    //HACER ESTO CON CUADRADOS INVISIBLESS CON TAM Y TODO
    if (mouseX > posxinv1 * width / 1000 && mouseX < posxinv1 * width / 1000 + taminvx1 * width / 1000 && mouseY > posyinv1 * height / 700 && mouseY < posyinv1 * height / 700 + taminvy1 * height / 700) {
    ...
    for (var i = 0; i < miEjercito2.length; i = i + 1) {
      ...
      if (dist(mouseX, mouseY, miEjercito2[i].x, miEjercito2[i].y) < 12) {
        estado = 12;
      }
    }
...
function torta(miX, miY) {
  ...
  this.dibujarse = function() {
    if (this.estaVivo == true) {
      image(micake, this.x, this.y, 50, 80);
    }
    this.moverse = function() {
      this.x = this.x + random(0 * width / 1000, 0 * width / 1000);
      this.y = this.y + random(-20 * height / 700, 20 * height / 700);
